...
{
    const CRLF = "\r\n";
    const CHECKURL = 'http://www.baidu.com/';
    ...
    public function set_basic_info($basic_info)
    {
    ...
    {
        if (is_array($input)) {
            return array_map(array('\Sys\Http\Curl', 'urlencode'), $input);
    ...
    {
        if (is_array($input)) {
            return array_map(array('\Sys\Http\Curl', 'urlencode_raw'), $input);
        } else if (is_scalar($input)) {
    ...
    {
        if (is_array($input)) {
            return array_map(array('\Sys\Http\Curl', 'urlencode_rfc3986'), $input);
        } else if (is_scalar($input)) {
    ...
    private function curl_setopt()
    {
        ...
        if (!empty($this->basic_info)) {
            ...
            curl_setopt($this->ch, CURLOPT_USERPWD, $this->basic_info);
            $this->curl_cli .= " -u " . $this->basic_info;
        }
